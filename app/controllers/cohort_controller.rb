class CohortController < ApplicationController
  ...
  def cohort
    ...
    @dead_ever = report.dead_ever.length

    @dead = report.dead.length

    @transfer_out_ever = report.transfer_out_ever.length

    @transfer_out = report.transfer_out.length

    @stopped_treatment_ever = report.stopped_treatment_ever.length

    @stopped_treatment = report.stopped_treatment.length

    @alive_ever = report.alive_ever.length.to_i - report.defaulters_ever.length.to_i - report.transfer_out_ever.length.to_i - report.stopped_treatment_ever.length.to_i

    @alive = report.alive.length.to_i - report.defaulters.length.to_i - report.transfer_out.length.to_i - report.stopped_treatment.length.to_i
    ...
    @defaulters_ever = report.defaulters_ever.length

    @defaulters = report.defaulters.length
